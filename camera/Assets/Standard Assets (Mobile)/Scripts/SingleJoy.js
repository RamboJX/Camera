...
static private var joysticks : SingleJoy;					// A static collection of all joysticks
...
private var fingerDownPos : Vector2;						//手指按下的位置
...
{	
	if ( !enumeratedJoysticks )
	{
		...
		joysticks = FindObjectsOfType(SingleJoy) as SingleJoy;
		...
		for(var i : int = 0;i < count; i++)
		{
			...
			{	
				...
				{	
					...
					///if(CtoS.flag==1)
					//{
						position.x = Mathf.Clamp( ( touch.position.x - fingerDownPos.x ) / ( touchZone.width / 2 ), -1.0, 1.0 );
						position.y = Mathf.Clamp( ( touch.position.y - fingerDownPos.y ) / ( touchZone.height / 2 ), -1.0, 1.0 );
					//}
				//	else
					//{
